...
def run_verify():
    ...
    for d in data:
        ...
        blob = (q.get("nl", "") + " " + e.get("nl", "") + " " + " ".join(o.get("nl", [])))
        if ARABIC_RE.search(blob):
            issues.append((i, "leftover Arabic in Dutch fields"))
        if len(o.get("nl", [])) != len(o.get("ar", [])):
            issues.append((i, "option count mismatch"))
        # length sanity: a near-empty Dutch explanation against a substantial Arabic one
        # signals dropped content. Conservative thresholds to avoid false positives.
        if len(e.get("ar", "")) > 60 and len(e.get("nl", "")) < 25:
